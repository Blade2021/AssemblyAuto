...
void loop() {
  ...
  if ((SaveButtonTrigger) && (Timer1 - buttonPreviousTime >= buttonWait)){
    ...
    if ((a>=1) && (Active==1)){
      ...
      if ((b == 1) && (Timer1 - previousTimer2 >= y[2])){
        previousTimer2 = Timer1;
        lcd.setCursor(0,1);
      ...
      if ((b == 2) && (Timer1 - previousTimer2 >= y[3])){
        previousTimer2 = Timer1;
        lcd.setCursor(0,1);
      ...
      if ((b == 3) && (Timer1 - previousTimer2 >= y[4])){
...
void setLEDB(byte LEDBnumber)
{
  digitalWrite(Led3, LOW);
  digitalWrite(Led4, LOW);
  digitalWrite(Led5, LOW);

  digitalWrite(LEDBnumber, HIGH);
}
